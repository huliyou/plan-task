import { create, remove, update, query,findByProb } from '../services/responsible'
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/probs/responsible') {
          dispatch({
            type: 'query',
  ...
  effects: {
    ...
    *update ({ payload }, { select, call, put }) {
      yield put({ type: 'hideModal' })
      const id = yield select(({ responsible }) => responsible.currentItem.id)
  ...
  reducers: {
    ...
    showModal (state, action) {
      const {dictItems,solveUser} = action.payload
      return { ...state, ...action.payload, solveUser,dictItems,modalVisible: true }
    },
    hideModal (state) {
